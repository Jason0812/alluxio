...
abstract class AbstractFileSystemThrough extends org.apache.hadoop.fs.FileSystem{
    ...
    public BlockLocation[] getFileBlockLocations(FileStatus file, long start, long len) throws IOException{
        ...
        if(MODE_CACHE_ENABLED){
            AlluxioURI mUri = new AlluxioURI(HadoopUtils.getPathWithoutScheme(file.getPath()));
            try {
                if(mFileSystem.exists(mUri) || mUserMustCacheList.inList(HadoopUtils.getPathWithoutScheme(file.getPath()))){
    ...
    public boolean rename(Path src, Path dst) throws IOException {
        ...
        if (MODE_CACHE_ENABLED){
            ...
            if(!srcInCacheList && dstInCacheList){
                ...
                LOG.error("Currently doest not support rename hdfs space to alluxiioi space.", src, dst);
                return false;
            }
